...
- **Gateway auth is unverified through the SDK.** The action's public input contract promises `anthropic-auth-token` + `ANTHROPIC_BASE_URL`, implemented today as a `claude-code-action#1294` workaround ([`action.yml:143-150`](../../../ai-review/action.yml#L143-L150)). The SDK equivalent must be proven before anything else is built. **If this does not work, the design does not ship** — it is the first thing to test.
